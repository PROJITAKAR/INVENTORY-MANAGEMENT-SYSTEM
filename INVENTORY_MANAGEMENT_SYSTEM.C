...
void purchaseprod(){// function for purchasing a product 
    ...
    for (i=0; i<count; i++){
		...
		printf("\nPrice: %.2lfphp\n\n",prod[i].price);
            ...
            float discount = quant*(prod[i].price*(prod[i].discount/100.0));
            prod[i].sales= (quant*prod[i].price) - discount;
...
void salesReport() {
    ...
    for (i = 0; i < count; i++) {
        if (prod[i].sales > 0) {
            printf("| %-10s | %-20s | %-10d    |$%-15.2f|\n", prod[i].id, prod[i].name, prod[i].numSold, prod[i].sales);
            totalSales += prod[i].sales;
        }
    }
